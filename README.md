...
В качестве аргументов каждый компонент из `View` требует объект _имплиментирующий_ интерфейс `IEvents`, таким образом каждый компонент заменяет _колбеки_ на общий событийный объект. Эти компоненты так же не имеют контроля над собственным состоянием, а принимают его через _пропсы_.
...
Предстваленный в виде класса `AppState`. Управляет (сохраняет) состоянием приложения
...
- `public toggleClass(element: HTMLElement, className: string, force?: boolean)` - Переключает класс для указанного элемента. При наличии параметра `force` принудительно добавляет или удаляет класс.
- `public addClass(element: HTMLElement, className: string)` - Добавляет класс указанному элементу.
- `public setClass(element: HTMLElement, className: string)` - Устанавливает (заменяет все существующие классы) указанный класс для элемента.
- `protected setText(element: HTMLElement, value: unknown)` - Устанавливает текстовое содержимое для элемента. Принимает значение любого типа, преобразуя его в строку.
...
**Примечание:** 
Класс `Basket` обеспечивает базовый функционал для управления корзиной покупок, включая отображение списка товаров, их общую стоимость и доступность кнопки покупки.
...
**Примечание:**
Класс `BasketItem` обеспечивает управление каждым элементом в корзине, предоставляя методы для настройки его характеристик и обработки удаления.
...
Класс `AppState` хранит в себе состояние всего приложения
...
- `__basketItems` - содержит в себе все элементы из каталога, представленные в виде `CatalogItemDto[]` добавленные в корзину.
- `__catalogItems` - содержит в себе все элементы каталога.
- `__orderForm` - содержит в себе данные формы `Order`.
- `__contactsForm` - содержит в себе данные формы `Contacts`.

**Сеттеры:**

- `catalogItems` - устанавливает элементы каталога в `__catalogItems` и запускает эвент `catalogItems:set`.
- `orderForm` - устанавливает форму `__orderForm`.
- `contactsForm` - устанавливает форму `__contactsForm`.

**Геттеры:**

- `catalogItems` - возвращает все элементы каталога из `__catalogItems`.
- `orderForm` - возвращает форму из `__orderForm`.
- `contactsForm` - возвращает форму из `__contactsForm`.
- `basketItems` - возвращает все элементы содержащиеся в корзине `__basketItems`.
- `basketTotal` - производит вычисление стоимости корзины.

**Методы:**

- `getCatalogItemById(id: number | string): CatalogItemDto | undefined` - возвращает элемент по `id` из `__catalogItems`.
- `addBasketItem(catalogItem: CatalogItemDto): void` - добавляет элемент каталога в корзину `__basketItems` и запускает эвент `basketItems:change`.
- `removeBasketItem(id: string): void` - удаляет элемент каталога из корзины `__basketItems` и запускает эвент `basketItems:change`.
- `clearBasket(): void` - очищает корзину и запускает эвент `basketItems:change`.
- `resetForms(): void` - очищает формы `__orderForm` и `__contactsForm`.
- `validateOrderForm(): FormProps` - валидирует форму `__orderForm`.
- `validateContactsForm(): FormProps` - валидирует форму `__contactsForm`.
